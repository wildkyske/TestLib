...
inline void MD5_Init(MD5Context * context)
...
inline void MD5_Update(MD5Context * context, unsigned char * buf, int len)
...
inline void MD5_Final(MD5Context * context, unsigned char digest[16])
...
static const int MD5_DEFAULT_LEN = 32;
static const int MD5_DEFAULT_BUFFER_SIZE = MD5_DEFAULT_LEN + 2;
inline void GetMD5Code(unsigned char* pBuf, int nSize, char pCode[MD5_DEFAULT_BUFFER_SIZE])
{
	MD5Context context;
	unsigned char buff[int(MD5_DEFAULT_LEN/2)];
	MD5_Init(&context);	
	MD5_Update(&context, pBuf, nSize);	
	MD5_Final(&context, buff);	
	for(int j = 0;j < int(MD5_DEFAULT_LEN/2); j++){		
